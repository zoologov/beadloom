...
«всё в порядке». Пять состояний были неотличимы от успеха, и теперь каждое называет себя:
...
| `sync-check` | Проверить синхронность документации и кода — вердикты `ok` / `stale` / `missing` / `unverified`; `--record-surface` фиксирует размер объявленной поверхности |
