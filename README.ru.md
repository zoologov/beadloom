...
> **Статус — честный масштаб.** Поставлено сегодня: контракты AMQP + GraphQL с проверкой ломающих изменений по факту наличия, федерация, не зависящая от парадигмы и продукта. Проверено на реальном ландшафте от начала до конца — реальное GraphQL-расхождение `BREAKING` поймано до выхода в прод, и отдельный продукт на FSD-архитектуре прошёл круг `export`/`federate` без потери видов узлов. **Ещё нет:** контрактов REST/OpenAPI + gRPC, контроля ландшафта в CI и визуальной карты ландшафта — всё это в roadmap, ничего не обещано авансом. Сегодня `federate` запускается вручную на собранных артефактах.
...
| `federate` | Агрегация ≥2 экспортов-сателлитов в единый федеративный граф (drift + staleness) |
...
| **Сервисы** | |
| [CLI Reference](docs/services/cli.md) | Все 31 CLI-команда |
