...
Чат-бот, который отвечает на вопросы и пишет куски кода по запросу, стал обычным делом. Это базовый уровень.

Дальше начинается другое. Агент сам разбирается в задаче, планирует, делит её на части, пользуется инструментами, пишет и проверяет код, правит документацию, работает вместе с другими агентами и выполняет длинные цепочки действий. Он занят этим долго и без участия человека.
...
Есть случай неприятнее. Агент выглядит согласным с правилами, но о части сделанного умалчивает, а часть проверок обходит. Это называют скрытым стратегическим поведением, scheming. Такие сценарии уже изучают в лабораториях. В обычной работе мы их обычно не замечаем: сначала не замечаем, потом разводим руками и говорим, что модель поглупела.
...
Так бывает чаще, чем кажется. В пути правила опечатка, и оно не совпало ни с одним файлом. Документ есть в графе, а с диска его удалили. Репозиторий только что склонировали, и проверке свежести не с чем сравнивать. Страж настроен верно, но никуда не подключён и ни разу не сработал.

Честный ответ тут один: «это я не проверил». Обычный ответ — зелёный.

Beadloom пишет, чего он не сделал. Вот строки из настоящих прогонов:
...
У стража шесть исходов вместо двух:
...
- `error` — страж отказывается толковать то, что ему дали. Тоже не проходит;
- `unresolved` — страж не смог проверить сам себя. Его код не импортируется, настройки не читаются.

Про `unresolved` стоит сказать отдельно. Если сломанный контролёр запрещает всё подряд, он запрещает и ту правку, которая его чинит. Получается ловушка, из которой агент не выберется сам. Поэтому здесь Beadloom предупреждает, а не блокирует.

Что именно каждая проверка отказывается утверждать, разобрано [ниже](#когда-проверить-не-удалось-beadloom-так-и-пишет).
...
| Policy as code | да | стражи как данные, правила области задачи, шесть вердиктов и коды возврата |
...
`beadloom init` сверяет граф, который только что записал, с правилами, записанными рядом. Если скелет нарушает одно из них, `init` называет правило и узел и завершается с кодом 1, а не с кодом 0 и разбирательством на первом же `beadloom ci`. Скелет в любом случае остаётся на диске. Документы, которые он пишет, так пока не проверяются, поэтому первый `beadloom ci` стоит прочитать, а не считать зелёным заранее.
...
| `impact REF_ID` | Кто ещё пишет в этот узел, кто его вызывает, сколько у него ветвлений |
...
| `guard --liveness` | Какие стражи есть и к чему каждый из них на самом деле подключён |
| `export` / `federate` | Выгрузить граф и собрать ландшафт из нескольких сервисов |
| `docs site` | Собрать портал на VitePress |

Полный справочник — **[docs/services/cli.md](docs/services/cli.md)**: там все команды со всеми флагами, включая `axes`, `typed-surface`, `bd-calls`, `issue-number`, `rooms`, `mutation`, `review-brief`, `docs spaces`, `snapshot`, `status --debt-report` и настройку хуков через `install-hooks`.
