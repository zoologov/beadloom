...
- **Кросс-репозиторная федерация** *(экспериментально, тонкий срез)* — `beadloom export` создаёт детерминированный артефакт по репозиторию; `beadloom federate` объединяет ≥2 репозиториев в единый граф через рёбра `@repo:ref_id`, с вердиктами drift (намерение vs реальность) и оценкой устаревания каждого сателлита. Узлы и рёбра несут поле `lifecycle` (`active`/`planned`/`deprecated`/`dead`)
