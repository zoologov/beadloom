...
Для AI-агентов `beadloom prime` собирает все три примитива в пакет <2K токенов — одна команда заменяет цикл grep→read→guess. А `beadloom docs site` публикует всё это как базу знаний VitePress — дашборд метрик AaC/DocAsCode, интерактивную архитектуру, 🌟 карту кросс-репо ландшафта и рукописные доки с бейджами актуальности по каждому документу (каждое число — из того же кода, что и гейт, который его измеряет).
...
| `docs site` | Сгенерировать базу знаний VitePress из графа (дашборд метрик + интерактивная архитектура + 🌟 карта кросс-репо ландшафта + опубликованные валидированные доки) |
...
| **Сервисы** | |
| [CLI Reference](docs/services/cli.md) | Все 34 CLI-команды |
...
| [VitePress Site](docs/guides/vitepress-site.md) | Публикация базы знаний VitePress (дашборд + карта ландшафта + валидированные доки) |
