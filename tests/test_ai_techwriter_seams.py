...
def test_ctx_json_returns_empty_on_non_object(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        commands, "run_command", lambda args, *, cwd: CommandResult(0, "42", "")
    )
    assert commands.beadloom_ctx_json(Path("/x"), "graph") == {}
...
def test_goose_runner_parses_usage_report(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    runner = GooseAgentRunner(
        project_root=Path("/x"), recipe_path=Path("recipe.yaml"), model="default-m"
    )
...
def test_goose_runner_defaults_when_no_usage_line(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        seams, "run_command", lambda args, *, cwd: CommandResult(0, "no json here", "")
    )
    runner = GooseAgentRunner(
        project_root=Path("/x"), recipe_path=Path("r.yaml"), model="default-m"
    )
    res = runner.run(_packet())
    assert res.rewritten_paths == ("docs/graph.md",)
    assert res.input_tokens == 0
    assert res.model == "default-m"
...
def test_goose_runner_skips_non_object_and_invalid_json_lines(
    ...
    stdout = f"{usage}\n{{not json}}\n[1, 2, 3]\n"
    monkeypatch.setattr(seams, "run_command", lambda args, *, cwd: CommandResult(0, stdout, ""))
    runner = GooseAgentRunner(
        project_root=Path("/x"), recipe_path=Path("r.yaml"), model="default-m"
    )
    res = runner.run(_packet())
...
def test_goose_runner_raises_on_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        seams, "run_command", lambda args, *, cwd: CommandResult(1, "", "kaboom")
    )
    runner = GooseAgentRunner(
        project_root=Path("/x"), recipe_path=Path("r.yaml"), model="m"
    )
