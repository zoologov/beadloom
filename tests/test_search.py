...
class TestSearchFts5:
    def test_basic_keyword_search(self, search_conn: sqlite3.Connection) -> None:
        from beadloom.search import search_fts5
...
class TestHasFts5:
    def test_has_fts5_with_data(self, search_conn: sqlite3.Connection) -> None:
        from beadloom.search import has_fts5
...
class TestEscapeFts5Query:
    def test_basic_words(self) -> None:
        from beadloom.search import _escape_fts5_query
...
class TestSearchCLI:
    ...
    def test_search_finds_results(self, tmp_path: Path) -> None:
        ...
        result = runner.invoke(
            main, ["search", "authentication", "--project", str(project)]
        )
    ...
    def test_search_json_output(self, tmp_path: Path) -> None:
        ...
        runner = CliRunner()
        result = runner.invoke(
            main, ["search", "OAuth2", "--json", "--project", str(project)]
        )
        assert result.exit_code == 0, result.output
    ...
    def test_search_no_results(self, tmp_path: Path) -> None:
        ...
        runner = CliRunner()
        result = runner.invoke(
            main, ["search", "zzzznothing", "--project", str(project)]
        )
    ...
    def test_search_no_db(self, tmp_path: Path) -> None:
        ...
        runner = CliRunner()
        result = runner.invoke(
            main, ["search", "test", "--project", str(project)]
        )
        assert result.exit_code != 0
