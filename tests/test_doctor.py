...
class TestFacadeSources:
    """A node sourced at a package ``__init__.py`` hides the package's symbols.

    BDL-UX #157: after a cohesion-driven decomposition the natural place for a
    ...
    symbol read (node page, architecture pop-up, debt report, the ``max_symbols``
    rule) sees an empty node — consistently, which is why nothing contradicts it.
    """

    def _add_symbol(self, conn: sqlite3.Connection, path: str, name: str) -> None:
        conn.execute(
    ...
    def test_facade_source_hiding_symbols_warns_with_the_gap(
        self, conn: sqlite3.Connection
    ) -> None:
        # Arrange: a package whose annotation sits in the re-exporting façade.
        conn.execute(
            "INSERT INTO nodes (ref_id, kind, summary, source) VALUES (?, ?, ?, ?)",
            ("debt-report", "feature", "Debt report", "src/pkg/debt_report/__init__.py"),
        )
        self._add_symbol(conn, "src/pkg/debt_report/scoring.py", "compute")
        self._add_symbol(conn, "src/pkg/debt_report/collect.py", "collect")
        conn.commit()

        # Act
        checks = run_checks(conn)

        # Assert: named, quantified, and actionable.
        facade = [c for c in checks if c.name == "facade_sources"]
        assert facade, "no facade_sources check was produced"
        assert all(c.severity == Severity.WARNING for c in facade)
        description = " ".join(c.description for c in facade)
        assert "debt-report" in description
        assert "2" in description
        assert "src/pkg/debt_report/" in description

    def test_facade_source_with_no_hidden_symbols_is_ok(
        self, conn: sqlite3.Connection
    ) -> None:
        """A façade that really is the whole package is not a finding."""
        conn.execute(
            "INSERT INTO nodes (ref_id, kind, summary, source) VALUES (?, ?, ?, ?)",
            ("tiny", "feature", "Tiny", "src/pkg/tiny/__init__.py"),
        )
        self._add_symbol(conn, "src/pkg/tiny/__init__.py", "only_symbol")
        conn.commit()
        ...
        facade = [c for c in checks if c.name == "facade_sources"]
        assert facade
        assert all(c.severity == Severity.OK for c in facade)

    def test_directory_source_is_never_a_facade_finding(
        self, conn: sqlite3.Connection
    ) -> None:
        """The recommended shape — a package directory — must stay clean."""
        conn.execute(
            "INSERT INTO nodes (ref_id, kind, summary, source) VALUES (?, ?, ?, ?)",
            ("reindex", "feature", "Reindex", "src/pkg/reindex/"),
        )
        self._add_symbol(conn, "src/pkg/reindex/full.py", "reindex")
        conn.commit()
    ...
    def test_facade_finding_never_fails_the_gate(self, conn: sqlite3.Connection) -> None:
        """Shipped as WARNING: an existing project must not go red on upgrade.

        Only ERROR-severity doctor checks fail `beadloom ci`, and this condition
        is pre-existing in any project that decomposed a module into a package.
        """
        conn.execute(
            "INSERT INTO nodes (ref_id, kind, summary, source) VALUES (?, ?, ?, ?)",
            ("hidden", "feature", "Hidden", "src/pkg/hidden/__init__.py"),
        )
        self._add_symbol(conn, "src/pkg/hidden/impl.py", "run")
        conn.commit()

        checks = run_checks(conn)

        assert not [c for c in checks if c.severity == Severity.ERROR]

    def test_module_level_file_source_is_not_a_facade(
        self, conn: sqlite3.Connection
    ) -> None:
        """A plain module source is unaffected by the check."""
        conn.execute(
            "INSERT INTO nodes (ref_id, kind, summary, source) VALUES (?, ?, ?, ?)",
            ("gate", "feature", "Gate", "src/pkg/gate.py"),
        )
        self._add_symbol(conn, "src/pkg/gate.py", "run_gate")
        conn.commit()
