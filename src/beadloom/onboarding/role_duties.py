...
def _marked_files(project_root: Path) -> Iterator[Path]:
    ...
    # Every shipped fragment, with no exclusion. There used to be one: the
    # `templates/agentic_flow/agents/*.md.txt` snapshot carried every marker its
    # composed role carried, so subtraction listed all five of them the moment a
    # role core first declared a duty — under a reason that says the duties in it
    # reach no role, which was false twice over. `beadloom-iur5` deleted the
    # snapshot, so the derivation no longer has its own output in its input and
    # no longer needs a rule to say so.
    candidates = [
