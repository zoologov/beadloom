...
def render_sidebar_ru(
    conn: sqlite3.Connection,
    *,
    docs_root: Path,
    has_getting_started: bool,
) -> str:
    """The RU-locale sidebar JS array — curated About-only (deterministic).

    Mirrors :func:`render_sidebar`'s ORDER and structure exactly, with two
    differences: the **About** entry links to the RU home ``/ru/`` (the only
    translated page), and the top-level leaf labels are Russian. Every non-About
    LINK still targets the EN routes (``/dashboard``, ``/architecture``, the node
    pages, ``/landscape``, ``/docs/``) — because only About is translated (Q2 =
    curated-About-only).

    The nested Architecture/Documentation groups are reused from the EN builders
    (so their links + nesting stay identical and link-safe); only each group's
    top-level header label is swapped for Russian, plus the top-level leaf labels.
    Interior tree labels (per-node / per-doc) stay English — relabeling the whole
    tree would duplicate the EN builders for no routing benefit (every interior
    link is an EN route anyway).
    """
    arch_group = render_architecture_group(conn).replace(
        'text: "Architecture",', 'text: "Архитектура",', 1
    )
    docs_group = render_documentation_group_from_dir(docs_root, collapsed=False).replace(
        'text: "Documentation",', 'text: "Документация",', 1
    )
    entries: list[str] = [f'{_INDENT}{{ text: "О проекте", link: "/ru/" }}']
    if has_getting_started:
        entries.append(
            f'{_INDENT}{{ text: "С чего начать", link: "/docs/getting-started" }}'
        )
    entries.append(f'{_INDENT}{{ text: "Дашборд", link: "/dashboard" }}')
    entries.append(arch_group)
    entries.append(f'{_INDENT}{{ text: "Карта ландшафта", link: "/landscape" }}')
    entries.append(docs_group)
    return "[\n" + ",\n".join(entries) + ",\n]"
...
def render_nav_config(conn: sqlite3.Connection, project_root: Path) -> str:
    ...
    Top nav is empty (``render_nav``); the EN sidebar is the ordered, link-safe
    tree from :func:`render_sidebar` (About / Getting Started / Dashboard /
    Architecture / Landscape map / Documentation). The module also exports the RU
    locale data (``navRu`` empty, ``sidebarRu`` curated-About-only) so the
    ``config.mjs`` shell can wire a ``ru`` locale whose only translated page is
    About — every other entry links back at the EN routes.
    """
    ...
    return (
        ...
        f"export const navRu = {nav_ru};\n\n"
        f"export const sidebarRu = {sidebar_ru};\n"
    )
