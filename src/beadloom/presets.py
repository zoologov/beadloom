...
_ENTITY_DIRS = re.compile(
    r"^(models?|entities|schemas?|types|dataclasses|orm|db|database)$"
)
_FEATURE_DIRS = re.compile(
    r"^(api|routes?|controllers?|handlers?|views?|endpoints?|graphql|grpc|rest)$"
)
_SERVICE_DIRS = re.compile(
    r"^(services?|core|engine|workers?|jobs?|tasks?|processors?)$"
)
...
MONOLITH = Preset(
    name="monolith",
    description=(
        "Single deployable: top-level dirs are domains, "
        "subdirs map to features/entities/services."
    ),
    dir_rules=(
...
MICROSERVICES = Preset(
    name="microservices",
    description=(
        "Independent services: top-level dirs are services, "
        "shared code becomes domains."
    ),
    dir_rules=(
        PresetRule(_ENTITY_DIRS, "entity", "high"),
        PresetRule(_FEATURE_DIRS, "feature", "high"),
