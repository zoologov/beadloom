...
# beadloom:component=context-builder
...
def suggest_ref_id(conn: sqlite3.Connection, ref_id: str) -> list[str]:
    ...
    """
    row = conn.execute("SELECT ref_id FROM nodes WHERE ref_id = ?", (ref_id,)).fetchone()
    if row is not None:
        return []

    all_ids = [r[0] for r in conn.execute("SELECT ref_id FROM nodes").fetchall()]
    if not all_ids:
        return []
...
def bfs_subgraph(
    ...
    while queue:
        ...
        for erow in conn.execute(
            "SELECT e.src_ref_id, e.dst_ref_id, e.kind FROM edges e WHERE e.src_ref_id = ?",
            (current_id,),
        ).fetchall():
            neighbors.append(
                (
        ...
        # Incoming edges.
        for erow in conn.execute(
            "SELECT e.src_ref_id, e.dst_ref_id, e.kind FROM edges e WHERE e.dst_ref_id = ?",
            (current_id,),
        ).fetchall():
            neighbors.append(
                (
...
def _check_sync_status(
    ...
    return [{"doc_path": r["doc_path"], "code_path": r["code_path"]} for r in rows]
...
def build_context(
    ...
    *,
    ...
    for rid in ref_ids:
        row = conn.execute("SELECT ref_id FROM nodes WHERE ref_id = ?", (rid,)).fetchone()
        if row is None:
