...
def _parse_node_matcher(data: dict[str, object], context: str) -> NodeMatcher:
    ...
    if kind_str is not None and kind_str not in VALID_NODE_KINDS:
        msg = (
            f"{context}: invalid kind '{kind_str}', "
            f"must be one of {sorted(VALID_NODE_KINDS)}"
        )
        raise ValueError(msg)
...
def _parse_deny_rule(
    name: str, description: str, deny_data: dict[str, object]
) -> DenyRule:
...
def _parse_require_rule(
    ...
    has_edge_to = _parse_node_matcher(
        has_edge_to_data, f"Rule '{name}' require.has_edge_to"
    )
...
def load_rules(rules_path: Path) -> list[Rule]:
    ...
    if version != RULES_SCHEMA_VERSION:
        msg = (
            f"rules.yml: unsupported version {version}, "
    ...
    for idx, rule_data in enumerate(rules_data):
        ...
        if has_deny == has_require:
            ...
            msg = (
                f"rules.yml: rule '{name}' must have exactly one of 'deny' or 'require'"
            )
            raise ValueError(msg)
...
def validate_rules(rules: list[Rule], conn: sqlite3.Connection) -> list[str]:
    ...
    for ref_id in sorted(ref_ids):
        row = conn.execute(
            "SELECT 1 FROM nodes WHERE ref_id = ?", (ref_id,)
        ).fetchone()
        if row is None:
            warnings.append(
                f"Rule references unknown ref_id '{ref_id}' "
...
def _get_node(
    ref_id: str, conn: sqlite3.Connection
) -> tuple[str, str] | None:
    """Return (ref_id, kind) for a node, or None if not found."""
    row = conn.execute(
        "SELECT ref_id, kind FROM nodes WHERE ref_id = ?", (ref_id,)
    ).fetchone()
    if row is None:
        return None
...
def evaluate_deny_rules(
    conn: sqlite3.Connection, rules: list[DenyRule]
) -> list[Violation]:
...
def evaluate_require_rules(
    conn: sqlite3.Connection, rules: list[RequireRule]
) -> list[Violation]:
...
def evaluate_all(
    conn: sqlite3.Connection, rules: list[Rule]
) -> list[Violation]:
