...
Tests live in `tests/` (flat, no subdirs). Naming: `src/beadloom/<domain>/<module>.py` → `tests/test_<module>.py`; CLI → `tests/test_cli_<command>.py`; integration → `tests/test_integration*.py`.

### Tools + commands
```bash
uv run pytest                                                   # all tests
uv run pytest --cov=src/beadloom --cov-report=term-missing      # with coverage
uv run pytest --cov=src/beadloom --cov-fail-under=80            # enforce the floor
