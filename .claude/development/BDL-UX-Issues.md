...
146. [2026-08-03] [HIGH] `component` nodes are absent from `sync-check` pairs entirely — their docs are NEVER checked for staleness, and the green result reads as "docs fresh" when it means "nothing was looked at"
...
144. [2026-08-03] [MEDIUM] `max_symbols` counts by source-path PREFIX without subtracting nested nodes — carving a subpackage into its own node does NOT relieve an oversized parent, so the natural remedy silently does nothing

    **Severity:** medium (steers teams away from the correct fix, toward raising the ceiling)
    **Command:** `beadloom lint --strict` (rule `max_symbols`)
    **Context:** Dogfood on a downstream DDD project. A vertical-module node hit its `max_symbols` budget. The obvious remedy — split the oversized module into two nodes — was attempted the obvious way first: declare a nested node on a subpackage (`source: <parent>/<sub>/`). Empirically (`rules/evaluators.py`, `SELECT COUNT(*) … WHERE file_path LIKE '<source>%'`) the parent's count was unchanged: the child's symbols are matched by the parent's prefix too, so they are counted **twice** and the parent gains nothing.
    **Issue:** A node's size is "everything under my path", with no notion of delegation to a nested node. This is defensible as a definition, but it makes the rule's own remediation path a dead end: the team either physically relocates the subpackage to a **sibling** directory (a large, risky refactor with import churn across the codebase) or — far more likely under deadline — raises the ceiling / adds an exclusion, which is exactly what the rule exists to prevent. Nothing in the violation message hints that a nested node will not help.
    **Expected:** Subtract symbols owned by nested nodes from the ancestor's count (a node delegates the part of its tree it has handed to a child), or at minimum: (a) warn on overlapping `source` prefixes at graph-validation time, and (b) say so in the violation remediation ("a nested node will not reduce this count; move the subtree or split the module").
    **Workaround:** Make the new node a **sibling package** rather than a nested one, and accept the import churn.
...
138. [2026-07-02] [MEDIUM] `docs generate` silently no-ops on a coarse graph; no monolith-drift guard; `component`→`DOC.md` not generated
    ...
    **Workaround (Bob-side):** re-annotate source with granular `beadloom:feature=`/`component=` tags + rebuild `services.yml` with feature/component nodes, then `docs generate` + migrate `architecture.md` sections (tracked as Bob-Issues #1–#4).
...
135. [2026-06-18] [HIGH] `beadloom init` crashes with `UnicodeDecodeError` on a repo containing binary assets / a stray virtualenv
