...
**Goal:** Beadloom supports mobile, cross-platform, and server-side development languages. Critical for dogfood project (secondary-system: React Native + Expo + routing C++ + messaging).
...
**Dogfood project:** secondary-system — React Native + Expo + native modules:
- routing Routing Engine (C++)
- messaging (C++/embedded)
...
| Languages: order? | Kotlin → Java → Swift → C/C++ → Obj-C | By dogfood project priority (secondary-system) |
