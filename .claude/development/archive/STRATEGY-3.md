...
### Phase F3 — Tool-Agnostic Enforcement Everywhere  ✅ DELIVERED (BDL-039, 2026-06-02)

> **Status: delivered.** F1/F2 detection gained teeth. Shipped: the **landscape gate** (`federate --fail-on <csv>`) — the F2 edge/contract verdicts become a CI gate via the pure `gate_failures(fed, fail_on)` + `SAFE_DEFAULT_FAIL_ON` / `NEVER_FAIL_VERDICTS` (`graph/federation.py`), always writing `federated.json` before exiting non-zero and refusing to arm a no-false-gate verdict (principle 3); **agent-actionable output** — `Violation.remediation` surfaced by `lint --format json`/`github` (GitHub `::error` annotations, fix included); **AgentConfigAsCode** (`config-check [--fix]`, `onboarding/config_sync.py`) — regenerate-and-diff over only the auto-managed `beadloom:auto-start`/`auto-end` regions, reusing the exact `setup-rules --refresh` generator (no parallel reimpl, principle 7); the unified **`beadloom ci`** gate (`application/gate.py`) composing reindex → `lint --strict` → sync-check → config-check → **doctor** → optional `federate --fail-on`, never short-circuiting, one exit code, honest PASS/FAIL/SKIP per step, uniform `--format rich|json|github`; and a reusable **composite GitHub Action** (`.github/actions/beadloom-gate`) + GitLab template, with a documented **pull-based hub pattern** for the cross-service landscape (satellites publish commit-SHA-tagged exports; a hub job pulls ≥2 + `federate --fail-on`). **F3 success metric met (§7): a CI gate blocks a boundary violation / cross-service `BREAKING` / drifted agent-config regardless of which tool or human wrote the code** — dogfooded on Beadloom's own CI, and AgentConfigAsCode caught a **real `AGENTS.md` drift on Beadloom itself** during the BEAD-04 dogfood. Purely additive — no EXPORT/FEDERATION/DB version bump. The registry/artifact plumbing for the hub is the satellites' ops (a documented pattern, not Beadloom-built); no SaaS hub, no satellite auto-bootstrap. Follow-up → F4 (visual landscape map). See `docs/guides/ci-setup.md` + `docs/domains/graph/features/federation/SPEC.md`.
...
### Phase F4 — Living Knowledge Base + Visual Landscape (DocAsCode + VitePress)  ✅ F4.2 + F4.3 DELIVERED (BDL-040, 2026-06-02) · HARDENED by F4.4 (BDL-041, 2026-06-02) · F4.1 = next follow-up epic

> **Status: F4.2 + F4.3 delivered.** Shipped **`beadloom docs site [--out DIR] [--federated FILE]`** — a deterministic VitePress generator that reads the indexed graph read-only and emits a knowledge-base content tree (never touching the source `docs/`), with three showcases: **(A)** the AaC/DocAsCode metrics **dashboard** (`dashboard.md` + `dashboard.data.json`, `application/site_dashboard.py`) — lint/debt/docs/doctor + optional federated rollup, every figure from the SAME code path as its gate (honest by construction, the dashboard cannot disagree with `beadloom ci`); **(B)** 🌟 the **cross-repo landscape map** (`landscape.md`, `application/site_landscape.py`) — the F2 federated contract graph as a clickable Mermaid diagram, edges labelled by their hub `ContractVerdict`/`EdgeVerdict` verbatim, a `classDef` health overlay, nodes linking to their intra-repo page; **(C)** the **published validated docs** (`application/site_published.py`) — the REAL `docs/**` copied verbatim with a per-doc `doc_sync` freshness badge injected into the COPY only (`fresh` / `stale — <reason>` / `untracked`, last-synced + source-coverage %), the source `docs/` NEVER mutated, marker-delimited so regeneration overwrites only the badge. A committed VitePress scaffold (`site/`) renders the tree (build output gitignored). **Beadloom produces, VitePress renders** — static, no live server, no LLM. **F4 success metric (§7) met by construction:** badges and dashboard numbers come from the same `doc_sync`/`lint`/`doctor` code paths the gate runs, so the published KB is fresh *because* the gate is, with human review only on drifted nodes (the F4.1 loop). **Dogfooded** by building Beadloom's own site end-to-end (real `vitepress build`, exit 0) — and the real build found+fixed two genuine generator bugs that unit tests could not see: `.DS_Store` pollution copied into the published tree and 24 dead intra-site links. That is the honesty principle paying off — only a real render proves the produced tree is correct (cf. F3's "AgentConfigAsCode caught a real `AGENTS.md` drift"). Purely additive — no EXPORT/FEDERATION/DB version bump; the landscape map is a Mermaid thin slice (Cytoscape/D3 deferred). Follow-up → **F4.1** (AI tech-writer in CI). See `docs/guides/vitepress-site.md` + `docs/domains/application/README.md` + the federation SPEC.

> **Status: hardened by F4.4 (BDL-041, 2026-06-02).** F4 shipped the generator, but a green `vitepress build` was hiding broken pages — so F4.4 closed the **render-correctness + dashboard-UX** gap. Fixed the two F4 Mermaid bugs at the source (landscape ids prefixed `n_<sanitized>` so a node named `graph` can't collide with the keyword; C4 emits a `Rel` only between declared nodes, dropping the undrawable root Rel that crashed `drawRels`) and added a **generation-time Mermaid validity guard** (`application/site_mermaid_guard.py`) that fails pytest/generation on those bug classes — so "build green ≠ renders ok" can never recur. Turned showcase (A) into a real interactive **ECharts dashboard** (`dashboard.data.json` + committed Vue widgets): a critical-first `AlertBanner` + `StatusCards`, gauges, category charts, **honest trends** (only real recorded points from an additive `.beadloom/metrics_history.json` append-log, sparse-honest, no fabrication) and a `recommendations` panel — the old verbose text dump removed (the widgets are the single presentation surface; data honesty stays in the gate-sourced JSON). Made every diagram **pan/zoom/fullscreen** (`DiagramViewer`), reworked the landscape into the repo's **local contract graph** (own-site renders one real `beadloom → vitepress-site` CONFIRMED edge) with **page-aware clicks** (no dead 404 links), and added human-labelled `part_of`-nested **nav trees**. **Dogfooded on Beadloom's own site** — a real `vitepress build` exits 0 and the render was browser-confirmed (no broken diagram, no console error, no dead click). Purely additive — no schema/version bump. **F4 and F4.4 ship together** (a published broken diagram = a published lie). **F4.1** (the AI tech-writer in CI) remains the next follow-up. See `docs/guides/vitepress-site.md` + `docs/domains/application/README.md` + `.claude/development/docs/features/BDL-041/`.
>
> **Follow-on: portal IA + bilingual About shipped (BDL-046, 2026-06-02).** Reshaped the generated portal's information architecture: **About = the README** as the landing (`/`), a single ordered EN sidebar (About / Getting Started / flat Dashboard / collapsed Architecture with the overview moved to `/architecture` / flat Landscape map / expanded Documentation led by a descriptive Overview), an empty top nav (theme toggle + search retained), and a **bilingual EN/RU About via an in-page `/` ↔ `/ru/` toggle** — VitePress `locales` was evaluated and dropped because its global `/x ↔ /ru/x` mapping translated the whole menu and 404'd off `/ru/`. Feature SPECs are now tracked via per-symbol `# beadloom:feature=<ref>` annotations, and the untracked badge was reworded to a neutral "📘 reference". Browser-confirmed on the deployed site. Purely additive — no schema/version bump. See `docs/guides/vitepress-site.md` + `.claude/development/docs/features/BDL-046/`.
...
**F4.1 — AI tech-writer in CI (OSS, efficiency-first)  ⏭️ NEXT FOLLOW-UP EPIC (deferred from BDL-040)**

> **Status: deferred — the named next follow-up epic.** F4.2/F4.3 ship the static KB + dashboard + landscape map *without* any model; the published-docs showcase (C) computes `doc_sync` badges, it does **not** rewrite prose. F4.1 is the remaining piece: orchestrate an **external** model (never bundle weights) to refresh *drifted* doc nodes in CI, scoped by `sync-check` / `docs polish --json`, opening a PR for team/human review. The loop + loop-invariant below are the spec for the `beadloom docs ai-refresh` driver this epic builds.
