...
| # | Question | Decision |
|---|---|---|
| Q1 | Where exactly does the project overlay live — `.beadloom/roles/`, `.beadloom/flow/`, or keyed inside `flow.yml`? | Pending |
| Q2 | Does the guard registry live in `application/guards/` or as its own domain? Guards read the graph and are orchestration, which argues for `application`; a separate domain argues for reuse outside the flow. | Pending |
| Q3 | Default `.feature` location for adopters — `tests/acceptance/{features,steps}/` as in the dogfood project, or configurable from the start? | Pending |
...
| Q5 | Mutation tool: adopt `mutmut` as the dogfood project did, and ship it as an optional extra with a documented scope, or leave the tool to the project and ship only the role duty? | Pending |
