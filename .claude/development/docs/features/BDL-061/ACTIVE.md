...
| .41 | ✓ done | S3: three more ambient-decode sites, plus an MCP test runner with no timeout |
...
| .51 | ✓ done | P2: three modules past 1000 lines with many responsibilities each, self-flagged and owned by nobody |
...
| .60 | ✓ done | Filed by `.39`: the backslash refusal makes the flow guard refuse every edit on a Windows harness, and its stated reason is false there. Measured by proxy with `PureWindowsPath` and pinned |
...
| .67 | ✓ done | The decode sweep: 29 narrow handlers judged per site, per `.42`'s framework. Depends on `.68` |
| .69 | ✓ done | Filed by `.16`: `_writing.md.txt` and `_writing.ru.md.txt` tell every role that `beadloom lint` reports the five section-quality checks. It reports none of them — measured: 68 lint findings, all `scenario_coverage`, and 156 doc-quality findings from the other command |
...
| .72 | ✓ done | Split out of `.17`: the `ROADMAP` and issue-log kinds, and counts a mechanism computes so the hand-written tally cannot return. `.19` and `.20` will need it; the DAG was NOT rewired — that is the owner's call |
