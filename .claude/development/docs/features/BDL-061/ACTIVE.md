...
image stays the same size forever; an unbounded record, a silently shrinking population and an
attestation that blesses files nobody read all accumulate. Two agents ran it in one working tree,
each committing only its own files under the merge slot: `.84`/`.86`/`.62` and `.85`/`.63`/`.56`.
...
| .41 | ready | S3: three more ambient-decode sites, plus an MCP test runner with no timeout |
...
| .51 | ready | P2: three modules past 1000 lines with many responsibilities each, self-flagged and owned by nobody |
...
| .68 | ✓ done | Filed by `.66`: the decode family has **29 live candidates** in `src/` and no mechanism. Half one is one ruff line — `PLW1514` bites on a probe and reports ZERO here, so enabling it costs nothing. Half two is an AST ledger keyed on the CALL, the shape `test_windows_dimension.py` already ships one test file over. |
...
| .60 | ready | Filed by `.39`: the backslash refusal makes the flow guard refuse every edit on a Windows harness, and its stated reason is false there. Measured by proxy with `PureWindowsPath` and pinned |
| .62 | ✓ done | Filed by `.14`: three reference-leg defects — `Example:` prose read as a scenario reference, an indented code block read while a fenced one is not, and an undecodable document reading as no intent. Each a strict `xfail`, so the fix reddens the suite |
| .63 | ✓ done | Filed by `.14`, fixed on `features/BDL-061-drift` by option (b): the population states its own reach. Option (a) is rejected with its reason — an evaluator that remembered its own past population would be a WRITER, the `#147`/`#189` shape. Widening to components is rejected too: excluding plumbing is the architecture model's own definition, and 24 more findings is a decision nobody took. Measured here: *this rule's population is 42 of 82 graph node(s) (kind=feature); the other 40 are outside it: component (28), domain (7), service (4), site (1)* — larger than the bead's 24, which counted components only. Two existing tests INVERTED on purpose, because *absent from the verdict* was also absent from the arithmetic |
| .65 | ✓ done | Owner decision: pay the `measurable-goal` debt. `.70` answered the re-scope question first, so the number to start from is **4 of 232**, not 154 of 235 |
...
| .67 | ready | The decode sweep: 29 narrow handlers judged per site, per `.42`'s framework. Depends on `.68` |
| .69 | ready | Filed by `.16`: `_writing.md.txt` and `_writing.ru.md.txt` tell every role that `beadloom lint` reports the five section-quality checks. It reports none of them — measured: 68 lint findings, all `scenario_coverage`, and 156 doc-quality findings from the other command |
...
| .72 | ready | Split out of `.17`: the `ROADMAP` and issue-log kinds, and counts a mechanism computes so the hand-written tally cannot return. `.19` and `.20` will need it; the DAG was NOT rewired — that is the owner's call |
...
| .84 | ✓ done | Filed in `.24`, fixed on `features/BDL-061-drift`: the ACTIVE reconcile was inert on this repository's own table and reported coherent over a comparison of zero rows. Four faults, three of them found while fixing the first — the id-form mismatch, a scan that gave up at the first `Bead`-headed table without a `Status` column, a `bd list` view holding 41 of 709 beads with every closed one missing, and a short id resolved across the whole tracker where eight beads are numbered `.17`. A run now states how many rows it RESOLVED of how many it read. First working run: 191 of 283 resolved, 56 rows corrected in 12 files, two of them beads CLOSED in the tracker and reading `Pending`/`Open` here |
| .85 | ✓ done | Filed in `.24`, fixed on `features/BDL-061-drift`: a FACT and a CLAIM were one `UPDATE`. The node-level `symbols_hash` is what the index saw and is carried forward for every pair, so `.78`'s sibling verdict clears once its cause is re-baselined; `baseline_source = attested` is a claim about a document somebody read and is written only inside the scope. Withholding it is not cosmetic — an `index_build` baseline is corroborated against git rather than trusted, so the bulk form used to switch the harder check off for documents nobody opened. `sync-update <ref> --yes` now claims the STALE pairs; `--pair` names a document, `--code` narrows to one file, `--all-pairs` is the deliberate whole-ref claim. The two axes were decided by measurement on this branch, not taste: one README was stale against two agents' files in one run. **This bead's own fixpoint: 7 revised, 7 attested — 1-for-1, against 1/27, 13/20, 26/56 and 15/10 earlier in the epic; `unverified` fell 107 → 0.** Filed `.89` (the MCP tool is the same hazard one layer over) |
| .56 | ✓ done | Filed from review `.3` m7, carried out of `.35`; fixed on `features/BDL-061-drift`. The firing record is bounded by RECORDS (2000), not bytes and not age: a byte cap truncates mid-record and an age cap makes a quiet month read like a dead guard. Rotation loses no count — the outgoing firings fold into a carried summary holding, per guard, count, answered, first, last and last outcome, which is every input `--liveness` reads, so `fired_count` and `never-fired` are unchanged across a rollover. What it costs is per-firing `why` text older than one generation, and `carried_count` says how much of a count rests on a summary rather than on readable lines. `--liveness` still parses the whole active file, now as a decision: with a cap it is bounded, and a materialised counter would be a second source of truth for a number the file already holds. `.35`'s ignore entry SAID the record is not rotated; that sentence was false as of this bead and was rewritten in four places, and the pattern widened to `guard-firings*.jsonl`. Filed `.88` (a write-once block keeps the old pattern) |
| .86 | ✓ done | Filed in `.24`, fixed on `features/BDL-061-drift`: a document that SHOWS the `watches` syntax opted itself into surface drift — the same anchoring class `.83` closed for `refs:`. Four documents were enrolled without declaring, not one. A declaration now opens a line outside a fenced block; what position cannot decide is stated rather than hidden. Reference documents 12 → 9, unattestable `surface_drift` entries 1 → 0 |
