...
| .62 | in progress | Filed by `.14`: three reference-leg defects — `Example:` prose read as a scenario reference, an indented code block read while a fenced one is not, and an undecodable document reading as no intent. Each a strict `xfail`, so the fix reddens the suite |
