...
3. **Slice 3 (EXPANDED 2026-06-11 — owner decision): full architectural-honesty coverage, in two parts.**
   - **S3a — taxonomy + coverage-lint:** add a **`component`** node kind (alongside domain/feature; for internal/infra building blocks that aren't user-facing features but still warrant a tracked doc — `db`, `graph/loader`, `ctx builder`, `code-indexer`, `git_activity`, …). Replace the sprawl-lint with a **COVERAGE lint**: every `src/` module MUST map to a node (feature/component/…) OR be on an explicit **exempt** list (docstring-only). Exempt criterion (tunable): `< N public symbols AND not backing a CLI command AND internal-only`; the exempt list lives in `rules.yml` (visible, not a silent escape hatch). Goal: no shadow code — nothing untracked, nothing documented-once-then-stale (every node's doc is sync-checked + AI-tech-writer-refreshed).
   - **S3b — classify everything:** classify ALL ~80 src modules into feature / component / exempt; create the nodes (services.yml + `# beadloom:feature=`/`component=` annotations) + SPEC/DOC skeletons; coverage-lint clean. Capabilities → features; infra blocks → components; trivial glue → explicit exempt. (Replaces the original "re-model onboarding + audit".)
   - Rationale: the README-dump was the shadow zone (a module in shared README prose is weakly tracked → drifts). "Document everything" is sustainable BECAUSE sync-check + the AI tech-writer keep it fresh automatically (the product's own thesis; esp. after BDL-052 speed).
