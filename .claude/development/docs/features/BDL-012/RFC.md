...
**Root cause:** `generate_rules()` generates `domain-needs-parent` with `has_edge_to: {ref_id: ROOT}`. Sub-domains have `part_of → parent_domain`, not `part_of → ROOT`. 25+ false positives on core-monolith.
