...
## Open Issues (dogfooding on core-monolith — Django + Vue monolith, 44 nodes)
...
6. [2026-02-13] [HIGH] `lint` rules too strict for hierarchical projects — `domain-needs-parent` rule requires `part_of` to root node (`core-monolith`), but sub-domains like `apps-accounts` are `part_of apps` (their parent domain), not `part_of core-monolith`. Produces 25+ false positive violations. Fix: `generate_rules()` should generate `has_edge_to: {kind: domain}` for sub-domains, or change to "every domain must have at least one `part_of` edge" (requires rule engine enhancement to support `has_edge_to: any`).
