...
**And the first fix was incomplete, which PR #60's re-run said in one number.** Both locale
legs passed and `tests (3.10)` went red on `assert len(not_entered) == 2` — the same defect
class one level up, in the two assertions beside the one that was rewritten. A fixture
declaring 3.10 and 3.11 leaves two legs outside a Darwin run and one outside a 3.10 run, so the
literal was a claim about the machine it was authored on. Both are now stated as arithmetic,
and the instrument that closes the class is `tests/test_room_dependent_assertions.py`: the
modules that judge a verdict's room, re-run in every room this project's legs describe, with
the room replaced and nothing else. It catches a room-dependent literal at home, in the room
where it is wrong — which no local run could do before, because every local run is in 0 of 21.
...
| `beadloom-0mdo.30` | Done | PR #60's two locale legs, red on two tests this epic wrote. **The CJK work-item folder:** the guard is `_the_filesystem_can_spell(key)` — `name.encode(sys.getfilesystemencoding())`, the codec `os.mkdir` will use — so the case steps aside only where the room cannot hold the name, and the skip says where it still fails: every UTF-8 filesystem, macOS unconditionally and the `tests`/`gate`/`site-build` legs of every PR. That is the INVERSE polarity of `test_ci_locale_dimension.py:298` and says so instead of borrowing its wording; the reason is ASCII because `-ra` prints it on the two legs whose stdout cannot encode the name. **The scenario:** `a run names the declared rooms it did not enter` no longer contrasts one state against another. The two declared legs differ only in the interpreter, so at least one is always not entered — true at 0-of-2 on a developer machine and at 1-of-2 on the 3.10 leg — and a third step judges what `entered` means, the half that had never executed anywhere because the step skipped on Linux. `TestTheScenarioIsJudgedInsideADeclaredLegToo` writes a workflow declaring THIS run's platform and interpreter, so one leg is genuinely entered in whatever room the suite runs in, macOS included. `test_bead14_s4_binding` was NOT weakened; it gained a static sibling that reads the step modules, because its runtime check only catches a skip in the room where the skip fires. 7 tests added, each seen red first: the static scan reddened on exactly `test_verdict_room_steps.py:86`, sabotaging the room guard to `sys.platform != 'linux'` reddened 2, blinding the entered judgement reddened 1. Green on the tree, foreground and unpiped: 8440 passed (baseline 8433, so +7 and nothing else moved), 11 skipped — UNCHANGED, so nothing new steps aside here — 1 xfailed, 249.50 s. `beadloom ci` rc 0, `ruff` clean, `mypy --strict` Success over 255 files targeting 3.10, 3.11, 3.12 and 3.13. Darwin arm64 · CPython 3.13.7 — 0 of the 21 declared rooms. **Then `tests (3.10)` was red, and the first fix was incomplete.** The two assertions BESIDE the rewritten scenario still said `== 2` over a fixture declaring 3.10 and 3.11 (`tests/acceptance/steps/test_verdict_room_steps.py:228`, `tests/test_gate_verdict_room.py:83`); the other two failures were collateral — the acceptance suite's exit code, and the sabotage test reading a failure that was not the sabotaged file's. Both counts are now the arithmetic they were always meant to be: every declared leg is entered or reported as not, at least one of two legs differing only in the interpreter is outside, and the number the verdict PRINTS is the number the census measured. `test_this_repository_is_not_in_any_of_its_own_ci_legs_unless_it_is` lost its `if platform.system() != 'Linux'` branch for the same reason: it asserted the interesting half only where this project's legs do not run. **The instrument, and it is the point of the fix:** `tests/room_simulation.py` replaces `current_room` in `pytest_configure` — before the test modules import, so a module holding `from ... import current_room` stands where the product stands — and `tests/test_room_dependent_assertions.py` re-runs the modules that judge a verdict's room in every room this project's legs describe (Linux/3.10, 3.11, 3.12, 3.13 and Darwin/3.13, derived from `ci.yml`). Its population is read, not listed, and its two exclusions are rules: a module deriving its expectation from `sys.version_info` pins the function the simulation replaces, and the runner does not run itself. `sys` is deliberately NOT patched — measured, replacing `sys.version_info` breaks pydantic's annotation evaluation and two MCP tests then fail at the seam of the simulation rather than on a finding. TESTS MUST BITE: the old literal, written out into a module of its own, passes at home and fails inside the leg. 25 tests added on top of the first 7. Green on the tree, foreground and unpiped: 8465 passed, 11 skipped, 1 xfailed, 246.19 s; `beadloom ci` rc 0, `ruff` clean, `mypy --strict` Success over 255 files targeting 3.10, 3.11, 3.12 and 3.13. One run reported `test_guards_parity`'s read-only guard red on `.beads/issues.jsonl`; it is BDL-UX #233 — this session's own `bd` writes inside the measurement window — and it passes with `bd` idle. **And a third instance, inside the fix for the class.** PR #60's third run: the locale legs and 3.12/3.13 green, `tests (3.10)` and `(3.11)` red on one test — the bite test that proves the simulation works. Its two halves were not equally true: the sabotage module FAILS under the fabricated room in every room, and it PASSES unsimulated only where the run is outside the legs the module itself declares. Those legs were 3.10 and 3.11, so the second half said 'this run is not on 3.10 or 3.11'. The arrangement was fixed rather than the assertion loosened: the sabotage now declares `2.7` and `3.0`, interpreters nothing can be running, so no real room is inside its legs and the fabricated room moves with it to `Linux/2.7`. A new arm asserts the sabotage passes in every room this project's legs describe, which is where the premise was false; restoring the old arrangement reddens exactly `[Linux/3.10]` and `[Linux/3.11]`, the two legs that were red, from a Darwin machine in one run. The class name went with it — `TestTheRoomSuiteHoldsInARoomThisRunIsNotIn` was itself a claim about where the run is. 8475 passed, `beadloom ci` rc 0, clean room over 1 file 258 passed. **What is still not measured is the interpreter:** the simulation fabricates the census's room, and the census compares os and python as strings without asking which interpreter executes, so a fabricated `Linux/3.10` and a real one are the same input to the code under test — which is a narrower claim than a green leg, and PR #60's fourth run is the verification. **The measurement not taken:** neither locale leg. CPython forces a UTF-8 filesystem encoding on macOS, so `LC_ALL=C` here does not reproduce the Linux room and the coordinator's 98-passed run said nothing about it; both claims about those legs rest on reasoning stated in the bead. |
