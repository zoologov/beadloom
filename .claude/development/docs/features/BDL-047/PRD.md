...
- [ ] **G10 — Trigger on push to main/master, not nightly (added 2026-06-10).** Docs must refresh as soon as code lands — a nightly schedule leaves a staleness window where the team + downstream agents read outdated docs. Both wrappers run `on push to main/master` (+ manual dispatch); the schedule is removed. Loop-safe: the agent's PR is merged by a human → that push finds 0 stale → no-op (no infinite loop; `concurrency` serializes).
- [ ] **G12 — Drift detection in CI via `sync-check --since <ref>` (added 2026-06-10, found by dogfood).** A fresh CI checkout reindexes from scratch → re-baselines everything → `sync-check` sees 0 stale, so baseline-relative drift (hash/symbols_changed) is invisible. Fix (chosen: A2, a reusable core primitive): `beadloom sync-check --since <ref>` computes drift relative to a **git ref** (baseline = the push's parent), mirroring `diff --since`. The harness/CLI/workflow pass the push parent (`github.event.before`, fallback `HEAD~1`). Also benefits the general `beadloom ci`/Gate (which has the same fresh-clone blindness). PROVEN by dogfood: local-baseline=1 stale, fresh-reindex=0 stale.
- [ ] **G11 — Package the VPS-runner provisioning into the product (added 2026-06-10).** `setup-ai-techwriter` also emits an idempotent `provision-runner.sh` (parameterized for github/gitlab) so any project gets a fast, easy runner start — hardened with the lessons learned (swap-first guaranteed, disk-space check, fail-hard on critical steps, min-spec note). Turns the manual VPS setup into one command.
