...
- **[P0] ✅ Trunk-based + PR-triggered AI tech-writer. SHIPPED (BDL-049, 2026-06-10).** Moves the AI tech-writer from `on: push` to `main` to **`on: pull_request` → `main`**, and the dev flow to **trunk-based development**. The agent now runs **once per PR** against `--since $(git merge-base origin/<base> HEAD)` and **commits its doc refresh onto the PR head branch** (`--target pr-branch`, `[skip ai-techwriter]` message) + posts a PR comment — code + docs in one reviewable PR. A **loop-guard** (bot author / `[skip ai-techwriter]` + the workflow's `AI_TW_SKIP` step) stops the agent's own push from re-triggering; `cancel-in-progress: true`. GitLab mirrors via `merge_request_event`. New **`beadloom setup-branch-protection`** makes `beadloom-gate` a **required check** on `main` (PR required, owner-mergeable). The vendored `.claude/` coordinator flow + `CLAUDE.md` §6 are re-vendored trunk-based. **This resolves the redundant-runs (13 runs/5h, 1h/768K-token re-refreshes), red-`main`, and orphan-doc-PR problems surfaced during BDL-047/048** (the F4.1 follow-up "the epic's own code introduced doc drift" + "main goes red"). **Honest framing:** no auto-merge — a human merges the PR; CI on the PR is the true enforcement; the agent's refresh is a proposal.
  - **Bootstrap follow-up (post-merge):** the live `pull_request` trigger + `main` branch protection only become active **after BDL-049 itself merges** (the workflow on `main` is what runs on future PRs; protection is applied once via `setup-branch-protection`). The dogfood proof (a deliberate-drift PR that exercises one-run + in-PR refresh + no second run + green main) is a **separate post-merge PR** (BDL-049 `.6`), not part of the BDL-049 merge.
  - _Source: PRD/RFC BDL-049; follow-ups from BDL-047 F4.1._
- **[P0] ✅ CI consolidation. SHIPPED (BDL-050, 2026-06-11).** Replaces the three independent PR workflows (`beadloom-gate.yml` / `tests.yml` / `ai-techwriter.yml`) with **one `.github/workflows/ci.yml`** on `pull_request → main`: `gate` ∥ `tests` (3.10–3.13 matrix) ∥ `site-build` (VitePress build) run in parallel, then `ai-techwriter` with **`needs: [gate, tests, site-build]`** (runs only when all three are green → **no Qwen tokens on a broken PR**). The AI tech-writer harness gained a **verdict `{ok, flagged, infra}`** (discriminator `tokens > 0`): a genuine unresolved doc drift (`flagged`) blocks the PR, but an **infra failure** (dead runner / exhausted `$30` quota / provider 5xx; `tokens == 0`) PASSES with a loud `::warning::` so it never freezes merges. `tests` lost its `paths:` filter (un-filtered + required); `site-build` is now a PR gate (**closes `beadloom-wozp`** — VitePress/mermaid/interpolation breaks caught before `main`); redundant `push: main` gate/tests runs removed (`deploy-site.yml` is the only `push: main` job); all actions Node24-compatible (**closes `beadloom-t7vn`**); branch protection now requires the **7 consolidated check-runs** (`gate`, `tests (3.10..3.13)`, `site-build`, `ai-techwriter`); GitLab mirrors via stages `verify → docs` with the same `needs`. **Honest framing:** still no auto-merge — a human merges; CI on the PR is the true enforcement. Dogfooded by this very PR.
  - _Source: PRD/RFC BDL-050; follow-ups from BDL-049._
- **[P0] ✅ Beadloom governs itself. SHIPPED (BDL-051, 2026-06-11).** Closes the graph-discipline gap so Beadloom's own architecture is honest-by-construction: a new **`component` node kind** (internal/infra building block with a `DOC.md`, the mirror of a `feature`'s `SPEC.md`) + a **`module-coverage` lint** (now `severity: error` — every `src/beadloom/**.py` module is a tracked node or explicitly exempt; **no shadow code**, no `# type: ignore`-style escape hatch). The AI tech-writer harness moved out of `tools/` into a first-class **`ai_agents/ai_techwriter` domain** that ships inside the wheel (adopters run `python -m beadloom.ai_agents.ai_techwriter` — **no Python vendoring**; a `core-no-import-ai-agents` rule keeps it a leaf consumer). ALL src modules were **classified** (21 new feature/component nodes + a minimal, visible `exempt` list; dir-source coverage lets a cohesive leaf like `tui/` stay one node). The MCP `checkpoint` / `complete_bead` process-tools now **maintain the `ACTIVE.md` status table** correct-by-construction. **Honest framing:** the lint is the enforcement — a new untracked module fails `beadloom ci`. Dogfooded by this very epic.
  - _Source: PRD/RFC BDL-051; follow-ups from BDL-050 (graph discipline + ai_agents + flow adoption)._
- **[P0] ✅ Usable doc-flow + role configurator. SHIPPED (BDL-052, 2026-06-14).** Makes the packaged flow **tool- and stack-agnostic** and adds a **local-primary** enforcement layer. (S1) a blocking **pre-push Beadloom Gate** hook (`install-hooks --pre-push`) runs the full `beadloom ci` on every push and blocks on red (`git push --no-verify` is the documented escape hatch) + the coordinator Gate-loop + explicit parallelism. (S2) restored + modernized the CORE `dev`/`test`/`review`/`tech-writer` role protocols. (S3) the **role configurator** — `.beadloom/flow.yml` (`architecture: ddd|fsd` + `stack` + `tools`) → `compose_role(CORE + architecture overlay + sorted stack overlays)` → per-tool **adapters** (`.claude/agents/*` + `.cursor/agents/*` + a Cursor orchestrator pointer), `config-check` byte-guarding each composed adapter; FSD at parity with DDD; **roles are composer-owned**. (S4) **symbol-level scope** for the AI tech-writer (a doc is rewritten only when it references a symbol whose body changed — unioning new-side edits AND old-side removed/renamed defs so a doc naming a deleted symbol is KEPT; conservative when attribution is unavailable; kills the god-file fan-out). (S5) the CI `ai-techwriter` job runs per-doc repair in a **bounded parallel session pool** (`HarnessConfig.max_parallel`, default 3) with per-session 429/5xx **back-off** + uv-dep/index **caches** (behaviour identical to sequential; folded in stale order). (S6) `active-sync --stage`. **Honest framing:** local authoring is tool-agnostic + Gate-enforced, but `beadloom ci` in CI (no `--no-verify`) remains the single source of TRUE enforcement; the pre-push Gate is a fast local catch. Dogfooded by this very epic.
  - **Follow-ups surfaced by the epic:**
    - **Orphaned-adapter lint.** `config-check`'s composed-adapter check iterates only the tools named in `.beadloom/flow.yml`; a tool dropped from a narrowed `flow.yml` leaves its previously-scaffolded adapter set (e.g. `.cursor/agents/*`) un-drift-guarded + un-recomposed. A follow-up bead tracks linting orphaned adapters.
    - **Graph-model FSD-layer support.** The role overlays add FSD as a methodology, but the graph model itself does not yet model FSD layers/slices as first-class node kinds (DDD packages remain the modelled structure). Noted in the BDL-052 S3 review.
    - **More stack/tool overlays.** Kotlin / Swift / C++ / React-Native / Apollo overlays (and any further tools beyond Claude Code + Cursor) are not yet shipped — the overlay set is `python`/`fastapi`/`javascript`/`typescript`/`vuejs` × `claude`/`cursor`.
  - _Source: PRD/RFC BDL-052; follow-ups from BDL-050/051 (flow adoption + AI-TW speed)._
