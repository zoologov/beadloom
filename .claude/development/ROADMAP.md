...
- **[P0] F4.1 — AI tech-writer in CI, on Goose.** Closed scoped loop: `sync-check` detects drift → the **Goose agent** repairs the drifted nodes (with tool-use: `beadloom ctx/why`, reading neighbouring code/diffs, checking examples) → re-check to fixpoint → PR/MR for review. Stack: **Goose (MCP-native agent coordinator) + Qwen3.7-Plus (external API) + Beadloom + Beads**; GitHub Actions + VPS runner.
...
- **[P0] Agentic-flow packaging.** `beadloom init --agentic-flow` (scaffold `.claude/`) + **MCP process-tools with teeth**: `task_init` (folder + valid bead-DAG), `bead_context` (ctx+why+CONTEXT+rules in one call), `complete_bead` (**gate that can refuse**: runs lint/sync-check/tests), `checkpoint`. Directly strengthens north-star (a) — makes the stack reproducible and tool-agnostic (any MCP client). _Source: REVIEW-2 §7/§8._
