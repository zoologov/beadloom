...
- **[P0] ⏭ NEXT — BDL-052: AI tech-writer speed.** Make the AI tech-writer **non-blocking + faster**: drop the agent off the PR critical path (don't block merge on its run), narrow the over-scoping in `cli.py` (it currently rebuilds more context than a per-doc refresh needs), add **per-doc parallelism** (refresh independent drifted docs concurrently instead of strictly serially), and a **context/packet cache** so re-runs don't re-shell `docs polish` / `ctx` / `why` from scratch. Planned next; `/task-init` to follow. <!-- TODO: verify exact BDL-052 scope once the task is initialized -->
