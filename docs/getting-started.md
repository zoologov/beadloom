...
2. **`/coordinator`** — orchestrate the waves, gated by bead dependencies. `beadloom waves <bead>...` decides which of the ready beads may run at the same time from the code they occupy, names each wave's gate owner and each bead's clean room — which `beadloom clean-room <bead>` then builds, deriving the directory from the bead and refusing one it did not create — and states the five media every wave shares whatever its width — the working tree, the commit gate, the landing order, the doc baseline and the tracker's id space:
