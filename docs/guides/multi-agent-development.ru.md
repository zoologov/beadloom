...
## Что процесс приобрёл после версии 2.0.0

К описанному выше циклу добавились четыре механизма. У каждого есть своё руководство; здесь сказано, зачем механизм нужен и где он стоит в процессе, — руководство не пересказывается.
...
Восемь шагов идут именно в этом порядке, а `federate` становится девятым, когда проект объявляет спутники. `sync-check = 0` доказывает **свежесть** — раздел документации ссылается на актуальные символы кода. Это не проверка качества текста: за корректность формулировок отвечает человек на ревью pull request-а. `lint --strict` проверяет не только границы архитектуры, но и отсутствие «теневого» кода (проверка `module-coverage` в режиме ошибки). Три документных шага между ними — то, что Gate приобрёл после версии 2.0.0: `docs audit` сверяет числа и версии, заявленные в прозе, с теми же числами, вычисленными по проекту; `docs quality` читает плановые документы по стандарту письма; `doc spaces` проверяет отношение между намерением, записанным в документе TO-BE, и документом AS-IS, который должен описывать построенное.

Каждый шаг сообщает не только то, что проверил, но и то, что **не** проверял. `docs quality` называет виды документов, которые не читает ни одна проверка; `doc spaces` называет эпики, не объявляющие ни одного узла, и эпики, неизвестные трекеру; `lint` уточняет число правил тем, сколько из них не смогли проверить ничего. Это и есть тезис описываемого здесь релиза, выраженный в выводе самого Gate: зелёный шаг, не проверивший ничего, не должен читаться как зелёный шаг, проверивший всё.
