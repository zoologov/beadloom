...
**В облаке GitHub/GitLab** лежат код, каталог `docs/**`, каталог `.beadloom/`, описание конвейера CI и открытые pull request-ы. Единый `ci.yml` запускается на каждый pull request в `main`: задания `gate` (вердикт `beadloom ci`), `tests` (матрица версий Python 3.10–3.13), `tests-locale` (тот же полный набор тестов под `C` и под `en_US.ISO-8859-1`), `tests-windows` (тот же полный набор тестов на `windows-latest`) и `site-build` (сборка сайта на VitePress) идут **параллельно** на облачных runner-ах. Задание `ai-techwriter` объявлено через `needs: [gate, tests, site-build]` и стартует **только если зелёные эти три** — сломанный pull request не тратит токены Qwen. Задание `tests-locale` в этот набор `needs:` намеренно не входит: это измерение среды, а не требование к работе агента, и слияние оно останавливает через защиту ветки. Отдельный `deploy-site.yml` — **единственное**, что запускается на `push: main` и публикует сайт на GitHub Pages. При строгом trunk-based ветка `main` зелёная по построению.
...
Защиту настраивает `onboarding/branch_protection.py`. По умолчанию команда применяет `DEFAULT_STATUS_CHECK_CONTEXTS` — набор из десяти проверок консолидированного `ci.yml`, который получает любой проект, развёрнутый по нашему шаблону:
...
**В этом репозитории живая защита требует семь из них** — двух контекстов `tests-locale` и контекста `tests-windows` в ней нет, и это проверено запросом `gh api repos/:owner/:repo/branches/main/protection`. Обе проверки локали были заведомо красными, пока задача `beadloom-mr2l.42` не исправила вскрытый ими текстовый ввод-вывод, — сейчас они зелёные; `tests-windows` (BDL-061.39) ожидаемо красная на первых запусках, потому что до неё в этом проекте ничто никогда не выполнялось на Windows: измерением является разница между ногами матрицы, а не цвет любой из них. Красная обязательная проверка при `strict: true` сделала бы слияние в `main` невозможным, поэтому `beadloom setup-branch-protection` идемпотентна, но перезапускать её здесь сейчас **нельзя**: она применит все десять контекстов и заблокирует любое слияние, пока нога Windows не станет зелёной. Запускать её следует после того, как каждая нога была наблюдаемо зелёной на реальном pull request, — либо передавать `--check` с тем набором проверок, который конвейер действительно может выполнить.
...
| Порядок заданий | `gate ∥ tests ∥ tests-locale ∥ tests-windows ∥ site-build` → `ai-techwriter` (`needs: [gate, tests, site-build]`) |
...
| Обязательные проверки | по умолчанию 10: `gate`, `tests (3.10..3.13)`, `tests-locale (C)`, `tests-locale (en_US.ISO-8859-1)`, `tests-windows`, `site-build`, `ai-techwriter`; в этом репозитории живут 7 — без `tests-locale` и `tests-windows`, пока каждая нога не станет наблюдаемо зелёной |
