...
> Документ описывает, **где что разворачивается**, **кто за что отвечает** и **как части системы взаимодействуют между собой** — в текущем, выпущенном состоянии.
>
> Хронология изменений: BDL-047 (первый AI tech-writer в CI) → BDL-048 (упаковка многоагентного процесса и инструменты для MCP) → BDL-049 (переход на trunk-based и запуск по pull request) → BDL-050 (сведение CI в единый `ci.yml` и система вердиктов) → BDL-051 («Beadloom управляет сам собой») → BDL-053 (когерентность трекера и файла `ACTIVE.md`) → BDL-052 (настраиваемый, не привязанный к инструменту агентный процесс и pre-push Gate) → BDL-057 (`docs audit` входит в Gate) → BDL-061 (guard-ы процесса, проектный слой в `.beadloom/flow/`, исполняемые сценарии и качество документов, три пространства документов, `beadloom waves` и `beadloom review-brief`) → BDL-062 (`summary` узла сверяется с числами, которые вычисляет проект, а документ — с соглашением о раскладке, которое граф уже соблюдает).
...
Beadloom закрывает этот разрыв: обновление документации становится частью обычного процесса разработки.
...
Начиная с версии 2.0.0 упакованный процесс не привязан ни к одному языку программирования, ни к одному инструменту: роли собираются из частей по конфигурации. Начиная с BDL-061 S3 у сборки есть четвёртый слой, который принадлежит вашему репозиторию и переживает обновление, — см. [руководство по проектным оверлеям](./project-overlays.md).
...
## Модель, применённая к собственному коду Beadloom (BDL-051)
...
- Все модули проекта классифицированы, и сама классификация проверяется, а не декларируется.

BDL-062 перенёс ту же идею с модулей на сами узлы. Теперь узел держат за то, что он документирует себя там, где этот граф документирует узлы из его области исходников, и за то, что в его `summary` нет числа, которому проект противоречит. Узел, которому документ действительно не положен, записывает это решение вместе с причиной прямо в граф, а `doctor` печатает причину, — и отсутствие, о котором кто-то принял решение, перестаёт читаться так же, как отсутствие, которого никто не разбирал.
...
## Что добавилось к циклу с тех пор

К описанному выше циклу добавились пять механизмов — после того выпуска, который впервые его упаковал. У каждого есть своё руководство; здесь сказано, зачем механизм нужен и где он стоит в процессе, — руководство не пересказывается.

**Guard-ы процесса: правила перестают быть прозой, которую модель вправе проигнорировать (BDL-061 S1).** Guard отвечает на один процессный вопрос об одной ситуации — *покрыта ли эта правка заявленной задачей?*, *происходит ли это вне защищённого ствола?* — и возвращает вердикт, по которому обвязка действует, ничего не разбирая. Условие объявлено в блоке `guards:` файла `.beadloom/flow.yml`, вычисляет его Beadloom, а адаптер инструмента не содержит собственной логики. `beadloom guard <имя>` возвращает `0` при пропуске или прохождении, `1` при предупреждении и `2` при блокировке. Один код различается между оболочкой и хуком обвязки, и различается намеренно: дефект в объявленной конфигурации или командная строка, которой нельзя воспользоваться, дают `3` в оболочке и `2` под `--hook`, потому что `3` не останавливает вызов инструмента, а **guard, который не смог ответить, никогда не должен читаться как прошедший**. `beadloom guard --liveness` сообщает, какие guard-ы срабатывали, а какие не защищают ничего. В этом репозитории работают `bead-claimed` и `working-branch`, оба на поставляемом уровне `warn`.

**`beadloom waves`: форма волны выводится из графа, а не угадывается (BDL-061 S6).** Трекер знает, какая бусина блокирует какую; и только граф архитектуры знает, какой код они занимают. `beadloom waves BEAD [BEAD ...]` решает, какие из названных бусин могут идти одновременно, по независимости объявленных ими областей узлов на уровне кода, и каждая разведённая пара несёт одну причину из закрытого словаря (`blocked_by_bead`, `unresolved_scope`, `shared_node`, `shared_file`, `dependency_edge`, `override_serial`). Четыре среды разделяются при любой выбранной форме — одно рабочее дерево, один хук pre-commit, одна база свежести документации, одно пространство идентификаторов трекера, — и у каждой есть вердикт на этапе планирования, который может вернуться как `failed`; среда, которую никто не измерял, возвращается как `unmeasured`, и это находка с кодом возврата 1, а не молчаливое прохождение. Код возврата `0` — чисто, `1` — есть находки, `2` — решить нельзя. Проверяется предусловие, измеренное до запуска волны; за поведением волны после запуска здесь не следит ничто. См. [руководство по параллельным волнам](./parallel-waves.md).

**Три пространства документов: проверяемое утверждение — это отношение, а не флаг статуса.** Каждый документ лежит в TO-BE (`PRD`, `RFC`, `BRIEF`, `CONTEXT`, `PLAN`), AS-IS (`SPEC`, `DOC`, `README`) или WORKING (`ACTIVE`). Имена намеренно не TODO и DONE, потому что здесь ничто не меняет статус: PRD не становится выполненным, а когда работа заканчивается, пишется *другой* артефакт — документ AS-IS. Флаг не с чем сверять: `status: done` истинен потому, что кто-то его напечатал, и никакое дальнейшее изменение кода не сделает его ложным. У отношения оба конца лежат на диске: *этот эпик записал намерение, его бусины закрыты, а у названного им узла до сих пор нет документа, описывающего построенное*. Это решает `beadloom docs spaces`, и это шаг `beadloom ci`. Документы AS-IS — те, что `sync-check` связывает с кодом; документы WORKING освобождены от проверки свежести **по объявлению**, а не по выводу из отсутствующей пары. См. [руководство по видам документов](./document-kinds.md).

**`beadloom review-brief`: ревьюер получает изменение и спецификацию, а не авторский рассказ о том и другом (BDL-061 S6).** Команда собирает задание, объявленную область, спецификационные документы графа, связанные сценарии `@bead:` и все изменённые файлы — и **удерживает комментарии самой бусины**, сообщая, сколько их удержала. Рассказ не уничтожается: `--release` печатает его, как только записан комментарий с вердиктом. Смысл именно в порядке: агент, который сначала прочёл вывод автора, склонен проверять этот вывод, а не изменение.

**Собственные метаданные графа проверяются против проекта (BDL-062).** До этого среза все правила читали *связи* графа: какой модуль дотягивается до какого узла, какому слою позволено импортировать какой. Поля, которые граф хранит о самом себе, не читал никто, и два из них были здесь неверны на протяжении трёх мажорных выпусков, ни разу не покраснев. `graph-summary-facts` вычитывает числовые и версионные утверждения из `summary` каждого узла — из той самой фразы, которую цитируют `ctx`, `prime`, сгенерированный сайт и каждый адаптер агента, — и сверяет каждое с тем же фактом, который проект вычисляет о себе. `doc-area-coherence` спрашивает, документирует ли узел сам себя там, где велит *собственное* соглашение этого графа, и выводит это соглашение из проверяемого графа, а не из записанной где-либо раскладки, — потому оно работает и на проекте с нарезкой по фичам, и на здешнем «пакет на домен». Оба правила — обычные записи в `.beadloom/_graph/rules.yml`, и оба сообщают `unverifiable` как отдельное состояние: графу, в чьих `summary` нет ни одного числа или чьи документы не сходятся ни на одном соглашении, скажут, что его **пропустили**, а не что его проверили и он чист. См. [модель архитектуры](./architecture-model.md).

Рядом с этими пятью `beadloom export` и `beadloom federate` распространяют тот же вопрос «намерение против реальности» на несколько репозиториев: каждый сервис публикует детерминированный артефакт с привязкой к коммиту, а хаб собирает два и более таких артефакта в федеративный граф с вердиктами по связям и по контрактам. К однорепозиторному циклу выше это не относится, и `beadloom ci` запускает `federate` только тогда, когда проект объявляет спутники.
...
**В этом репозитории живая защита требует семь из них** — двух контекстов `tests-locale` в ней нет, и это проверено запросом `gh api repos/:owner/:repo/branches/main/protection`. Обе проверки локали были заведомо красными, пока задача `beadloom-mr2l.42` не исправила вскрытый ими текстовый ввод-вывод, — сейчас они зелёные: измерением является разница между ногами матрицы, а не цвет любой из них. Красная обязательная проверка при `strict: true` сделала бы слияние в `main` невозможным, поэтому `beadloom setup-branch-protection` идемпотентна, но перезапускать её здесь следует только после сверки объявленных контекстов с тем, что реально становится зелёным на открытом pull request, — либо передавать `--check` с тем набором проверок, который конвейер действительно может выполнить.

Число менялось в обе стороны, и именно это, а не сама величина, стоит запомнить. Десятым контекстом в BDL-061.39 добавили `tests-windows` — ногу, которая варьирует ПЛАТФОРМУ так же, как `tests-locale` варьирует окружение, — а в задаче `beadloom-mr2l.64` владелец её снял: нога измерена в ~16-28 минут runner-времени на каждый pull request и, в отличие от проверок локали, становится критическим путём конвейера, увеличивая время от pull request до слияния примерно втрое, — ради платформы, которой нет среди целевых для этого проекта. В этом проекте ничто никогда не выполнялось на Windows и, по этому решению, выполняться не будет; так и записано в `docs/domains/application/features/flow-guards/SPEC.md` — *не проверено по решению*, вместо того чтобы зелёный конвейер намекал на поддержку.
...
flowchart LR
  subgraph CORE["src/beadloom/ — ядро"]
    ...
    BP["branch_protection.py<br/>9 объявленных контекстов, 7 живых здесь"]
...
Эта возможность решает похожую проблему, только уже не для кода и документации, а для состояния задач. Процесс ведёт его в двух местах — в трекере `bd` и в таблице статусов внутри `ACTIVE.md`. Раньше оба поддерживались вручную и постепенно расходились с реальностью. `beadloom active-sync` делает их согласованными по построению.
...
Восемь шагов идут именно в этом порядке, а `federate` становится девятым, когда проект объявляет спутники. `sync-check = 0` доказывает **свежесть** — раздел документации ссылается на актуальные символы кода. Это не проверка качества текста: за корректность формулировок отвечает человек на ревью pull request-а. `lint --strict` проверяет не только границы архитектуры, но и отсутствие «теневого» кода (проверка `module-coverage` в режиме ошибки). Три шага из восьми — о документах, а не о коде: `docs audit` сверяет числа и версии, заявленные в прозе, с теми же числами, вычисленными по проекту; `docs quality` читает плановые документы по стандарту письма; `doc spaces` проверяет отношение между намерением, записанным в документе TO-BE, и документом AS-IS, который должен описывать построенное.

Каждый шаг сообщает не только то, что проверил, но и то, что **не** проверял. `docs quality` называет виды документов, которые не читает ни одна проверка; `doc spaces` называет эпики, не объявляющие ни одного узла, и эпики, неизвестные трекеру; `lint` уточняет число правил тем, сколько из них не смогли проверить ничего. `docs audit` сообщает три популяции вместо одной — факты, которые документ заявил и прогон рассудил; факты, для которых этот проект не объявляет значения, вместе с причиной отказа; и факты, которые проект вычисляет, но не заявляет ни один документ, — так что факт, покидающий знаменатель, называется по имени, а не вычитается молча. Это и есть тезис всей описываемой здесь линии работ, выраженный в выводе самого Gate: зелёный шаг, не проверивший ничего, не должен читаться как зелёный шаг, проверивший всё.

Строгость этого сообщения намеренно неодинакова, и полезна именно её несимметричность. Правило, у которого омертвела *часть*, сообщает предупреждение, каким бы уровнем оно ни было объявлено: дефект конфигурации не должен красить конвейер на обновлении, не изменившем ни строки кода. Правило, которое не смогло проверить **ничего** из своей популяции, — это другой факт: «нарушений не найдено» и «правило не работало» дают тогда один и тот же вывод, а повышение уровня, испаряющееся ровно тогда, когда правило перестало работать, — это повышение, которого нет. Поэтому `doc-area-coherence` сообщает о полном отказе от проверки на том уровне, который объявил проект. Пока так поступает только оно, и [модель архитектуры](./architecture-model.md) называет то правило, которое так не поступает.
...
| Обязательные проверки | по умолчанию 9: `gate`, `tests (3.10..3.13)`, `tests-locale (C)`, `tests-locale (en_US.ISO-8859-1)`, `site-build`, `ai-techwriter`; в этом репозитории живут 7 — без двух контекстов `tests-locale` |
...
- [`parallel-waves.md`](./parallel-waves.md) — что гарантирует волна параллельных агентов, какие четыре среды она разделяет при любой форме и как изолирован ревьюер
- [`document-kinds.md`](./document-kinds.md) — три пространства документов, архитектурные документы, которые держат против кода, и плановые документы, которые держат против стандарта письма
- [`project-overlays.md`](./project-overlays.md) — четыре слоя сборки и проектный слой, переживающий обновление
- [`architecture-model.md`](./architecture-model.md) — домен, фича и компонент, проверка без теневого кода и два правила, читающие собственные метаданные графа
