...
- `json` -- structured output: a backward-compatible `violations` array (now with an additive `remediation` key), a stable agent-actionable `findings` array (`{kind, rule, severity, node, locations, why, remediation}`), a `suppressed` array naming every crossing a `forbid_import` exemption excused, and a `summary` object (whose `violations_suppressed` is that array's length, and whose `layer_populations[]` states how much of its edge set each declared layer rule judged — `rule`, `edge_kind`, `evaluated`, `total`, `skipped_untagged`, `inherited_evaluated`, `inherited_total`, `unjudged`; additive, every key that was there keeps its name, BDL-070 A3). Deterministic (violations are pre-sorted).
- `porcelain` -- one colon-separated line per violation (default when piped), led by one `# layer_population:<rule>:<edge_kind>:<evaluated>:<total>:<skipped>:<inherited>` line per declared layer rule. The `# ` marker is the one `scope-check --porcelain` already leads its verdict with, and no rule name can begin with it, so a consumer that drops the marked lines reads exactly the seven-field records it read before (BDL-070 A3).
...
**A layer rule states how much of its edge set it judged, and it still judges the same ones.** `architecture-layers` resolves each end of an edge through the node's OWN declared layer tags and passes over one whose ends carry none, so the green line `0 violations, N rules evaluated` said the same words for a rule that had looked at a fraction of the graph as for one that had looked at all of it. Every run now states that fraction — on the rich summary line, GREEN and RED alike, and in each of the other three formats' own idiom (BDL-070 A3):

```text
0 violations, 16 rules evaluated, architecture-layers judged 16 of 365 live depends_on edge(s)
```

Measured on this repository on 2026-09-13 over a warm full rebuild of the index; `part_of` inheritance would reach 357 of the same population. The clause is printed at FULL reach as well as partial, because a population is the denominator of the counts beside it; the `layer_population` FINDING behind it is silent at full reach, because a finding is something a person triages on every run. **This is a report and not a verdict change**: inheritance has not shipped, the rule still decides on a node's own tags, the statement is `warn` whatever severity the rule declares, and a project that declares no layer rule gets no clause. The figure depends on how the index was built — a carried-forward index and a fresh one resolve two import rows differently (BDL-UX #290) — so hold the lineage constant across a before/after comparison.
...
`Measured on bd 1.0.4` states the release of the tracker and `The current release is 5.0.0`
states this project's, and each number in `bd 1.0.4 answers and beadloom 5.0.0 asks` goes to
