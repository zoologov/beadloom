...
- `init(*, bootstrap, preset, import_path, init_mode, non_interactive, force, project)` -- project initialization; also appends Beadloom's generated working set to the project's `.gitignore`, once. Four branches write a file under `.beadloom/_graph/`. The three that write a **bootstrap** graph — `--yes`, `--bootstrap` and the default interactive wizard — end by running the Gate's `lint_step` over it and exit 1 when it does not pass, so those branches cannot report success over a graph that fails the rules on disk (BDL-067, closing BDL-UX #192; the wizard was added in `.6`, having been missed while the covering tests counted the two bindings of `bootstrap_project` rather than the three branches). The fourth is `--import`, which writes `imported.yml` and takes no verdict: it re-indexes nothing and tells the adopter to run `beadloom reindex` next, so there is no index of its own output to judge. `--yes --mode import` likewise takes none — both of the verdict's headlines open with *the graph this command just wrote*, and that run wrote no bootstrap graph and no rules; it cannot meet an adopter's rules file either, because `--yes` returns `skipped` over an existing `.beadloom/` and `--force` deletes it. Two carve-outs remain named rather than implied: the wizard's `edit` review answer, where the graph has just been handed to the user to edit by hand and nothing has re-indexed since; and `--yes --mode both`, which is judged — its reindex was moved to the end of `non_interactive_init` in `.14`, because from inside the bootstrap block it produced an index that predated the `imported.yml` the same run went on to write, and `init` reported clean over it while the adopter's next `lint --strict` exited 1
