...
- `handle_complete_bead(project_root, *, bead, run_tests=True)` -- the refusing gate: `run_ci_gate` (+ tests); PASS closes the bead (`bd close --suggest-next`), confirms the suggestion against `bd ready --limit 0` through `bd_seam.answers.confirmed_suggestion`, and best-effort flips its ACTIVE.md table row to `✓ done`; FAIL returns findings and does NOT close (table untouched); both verdicts carry the room they were taken in; advisory-strong (CI is the true gate)
