...
- `generate_agents_md(project_root)` -- generate `.beadloom/AGENTS.md` with 14 MCP tools and rules; preserves content between `<!-- beadloom:custom-start -->` / `<!-- beadloom:custom-end -->` HTML comment markers (auto-migrates old `## Custom` format)
