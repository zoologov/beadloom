...
- **ai_techwriter_setup.py** — Backs `beadloom setup-ai-techwriter --platform {github,gitlab}` (BDL-047 / F4.1, G8). `scaffold(target_root, platform=...)` idempotently provisions the AI tech-writer into any repo: `vendor_harness()` copies the deterministic harness package (the `HARNESS_MODULES` `.py.txt` package-data assets) + the Goose recipe into `tools/ai_techwriter/` so `python -m tools.ai_techwriter` resolves on a target repo with no Beadloom source tree (the harness is repo tooling, not in the wheel); `_scaffold_github()` / `_scaffold_gitlab()` drop the chosen platform's CI wrapper (GitLab appends job-only to an existing `.gitlab-ci.yml`, never blindly clobbering, and skips an already-wired file); `_scaffold_provision_runner()` drops a hardened, idempotent `provision-runner.sh` (executable, `0o755`) into `tools/ai_techwriter/` — parameterized (`--platform/--repo/--token`, also env vars), `set -euo pipefail`, fail-hard RAM (~2 GB min, ~4 GB recommended) + disk (~5 GB) prechecks, swap guaranteed *before* apt (the OOM lesson), GitHub Actions runner *or* GitLab Runner registration as the critical step, and best-effort + verified Goose/beadloom/bd installs reported at the end (vendored from the `provision-runner.sh.txt` package-data asset); `_scaffold_guide()` writes the ≤3-step `docs/guides/ai-techwriter.md`. `sync_vendored_harness(live_root)` refreshes the package-data assets from the live `tools/ai_techwriter` source (drift guard, principle 5 — asserted byte-for-byte in tests). `vendored_harness_root()` / `templates_root()` locate the packaged assets.
...
- **branch_protection.py** — Backs `beadloom setup-branch-protection` (BDL-049). An idempotent `main`-branch-protection helper for the trunk-based flow (CLAUDE.md §6): every change integrates via a PR (no direct push) and the always-on `beadloom-gate` check is a **required status check**, so the gate becomes true enforcement (hardening BDL-048 G5). `build_protection_payload(*, status_check_contexts=DEFAULT_STATUS_CHECK_CONTEXTS)` builds the GitHub request body — `required_status_checks {strict: true, contexts}`, `enforce_admins: false`, `required_pull_request_reviews {required_approving_review_count: 0}`, `restrictions: null` — so a PR IS required but the **solo owner is never locked out** (can self-merge). `DEFAULT_STATUS_CHECK_CONTEXTS = ("beadloom-gate",)` is the **real** always-on check-run name; a required context MUST match a real check-run name EXACTLY and must NOT be a path-filtered workflow's check (under `strict` it would never run → permanently-unmergeable PR/`main`). `BranchProtectionRequest` (frozen dataclass: owner/repo/branch/`status_check_contexts`) exposes `endpoint()`, `payload_json()` (deterministic `sort_keys`), and `gh_args()` (the `gh api --method PUT … --input -` argv). `apply_branch_protection(owner, repo, *, branch="main", status_check_contexts=..., runner=None)` builds the declarative `PUT .../protection` and runs it through an injectable `GhRunner` seam (defaults to the real `gh` CLI; tests pass a fake that records argv + stdin without touching GitHub). `PUT .../protection` is declarative, so re-running re-settles the same state (idempotent).
...
beadloom setup-branch-protection --repo OWNER/NAME            # protect main: PR required + beadloom-gate a required check (GitHub)
...
- `scaffold(target_root, platform=...)` -- vendor harness + recipe, drop the platform CI wrapper, write the getting-started guide (idempotent); raises `ValueError` on an unknown platform
- `vendor_harness(target_root)` -- copy the vendored harness package data into `target_root/tools/ai_techwriter`
- `_scaffold_provision_runner(target_root)` -- drop the hardened, idempotent, executable `provision-runner.sh` (swap-first, RAM/disk prechecks, GitHub/GitLab runner registration) into `tools/ai_techwriter/`
- `sync_vendored_harness(live_root)` -- refresh the packaged `.py.txt` assets from the live harness source (drift guard)
- `vendored_harness_root()` / `templates_root()` -- locate the packaged scaffold assets
- `HARNESS_MODULES` / `PLATFORMS` -- the vendored module names + supported CI platforms
...
- `build_protection_payload(*, status_check_contexts=DEFAULT_STATUS_CHECK_CONTEXTS)` -- build the GitHub branch-protection request body (PR required, `beadloom-gate` required under `strict`, `enforce_admins: false`, 0 required reviews, `restrictions: null` → owner never locked out)
...
- `DEFAULT_STATUS_CHECK_CONTEXTS = ("beadloom-gate",)` / `DEFAULT_BRANCH = "main"` -- the real always-on required check + the default trunk
