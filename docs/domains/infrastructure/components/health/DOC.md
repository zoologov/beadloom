...
Computes and persists health snapshots (node/edge/doc counts, coverage %,
stale-PAIR and isolated-node counts) and derives trend indicators across snapshots — the honest,
...
Snapshots persist across reindexes. `stale_count` is not queried here: it is
`repository.count_stale_pairs(conn).count`, the one body that answers how many
stale PAIRS there are and carries the noun with the number (BDL-069
`beadloom-rqma.6`).
