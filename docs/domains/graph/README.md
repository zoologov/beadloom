...
- `render_federation_report(fed: FederatedGraph) -> str` -- Human-readable text report (satellites + sha/age, edge-verdict counts, DRIFT list, contract-verdict counts + explicit BREAKING / DRIFT / ORPHANED_CONSUMER / UNDECLARED_PRODUCER call-outs with the missing GraphQL names, unresolved refs).
...
- `reconcile_contracts(edges: list[dict]) -> list[Contract]` -- Group AMQP **and** GraphQL contract-bearing edges by `contract_key` into first-class `Contract`s; accumulates the producer `exposed` surface and consumer `references` (sorted + deduped) and folds the most-significant edge `lifecycle` (`external` > `dead` > `deprecated` > `planned` > `active`) onto each `Contract`, then assigns its `verdict` via `classify`. Insertion order preserved (the hub sorts the projected dicts by `contract_key`).
- `classify(contract: Contract) -> ContractVerdict` -- Assign the contract-level intent-vs-reality verdict (RFC §5, G5), lifecycle intent first: `external` → `EXTERNAL` (defensive; trigger wired in BEAD-05), `dead` → `DEAD`, `planned`/`deprecated` → `EXPECTED`; then shape: GraphQL with both sides and `references ⊄ exposed` → `BREAKING`, both sides compatible → `CONFIRMED`, consumers-only → `ORPHANED_CONSUMER`, producers-only → `UNDECLARED_PRODUCER`. Complementary to the edge-level `EdgeVerdict.UNDECLARED` (an additional projection, not a replacement).
